...
architecture behave of suma_BUS_A_s_axi is
    ...
    signal int_c_i             : UNSIGNED(31 downto 0) := (others => '0');
    signal int_c_o             : UNSIGNED(31 downto 0) := (others => '0');
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_C_I_DATA_0 =>
                        rdata_data <= RESIZE(int_c_i(31 downto 0), 32);
                    when ADDR_C_O_DATA_0 =>
                        rdata_data <= RESIZE(int_c_o(31 downto 0), 32);
                    when ADDR_C_O_CTRL =>
                        rdata_data <= (0 => int_c_o_ap_vld, others => '0');
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_C_I_DATA_0) then
                    int_c_i(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_c_i(31 downto 0));
                end if;
            end if;
        end if;
    end process;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (c_o_ap_vld = '1') then
                    int_c_o <= UNSIGNED(c_o); -- clear on read
                end if;
            end if;
        end if;
    end process;
